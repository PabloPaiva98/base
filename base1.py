...
# Função para calcular o rendimento desejado
def rendimento_desejado(codigo, valor_para_investir):
    if codigo in df['CÓDIGO'].values:
        ...
        st.write(f"O código {codigo} não está presente no DataFrame.")

# Menu suspenso para selecionar o código para calcular rendimento desejado
codigo_selecionado_investir = st.sidebar.selectbox('Selecione o Código para Investir', [''] + list(df_filtrado['CÓDIGO'].unique()))

if codigo_selecionado_investir != '':
    valor_preco = df.loc[df['CÓDIGO'] == codigo_selecionado_investir, 'COTA'].values[0]
    valor_rendimento = df.loc[df['CÓDIGO'] == codigo_selecionado_investir, 'DIVIDENDO'].values[0]

    st.write(f"Preço atual para {codigo_selecionado_investir}: R$ {valor_preco:.2f}")
    st.write(f"Dividendo para {codigo_selecionado_investir}: R$ {valor_rendimento:.2f}")

    valor_para_investir = st.sidebar.number_input("Digite o valor para investir:", min_value=0.0)

    if st.sidebar.button("Consultar"):
        rendimento_desejado(codigo_selecionado_investir, valor_para_investir)

# Função para adicionar códigos à carteira
def adicionar_a_carteira(codigos, valor_para_investir, carteira):
    for codigo in codigos:
        if codigo in df['CÓDIGO'].values:
            ...
            carteira.append({
                "CÓDIGO": codigo,
                "COTA": valor_preco,
                "DIVIDENDO": valor_rendimento,
                "Número de Cotas": numero_de_cotas,
                "Rendimento Total": rendimento_total
            })
        else:
            st.write(f"O código {codigo} não está presente no DataFrame.")

# Lista para armazenar a carteira
carteira = []

# Caixa de seleção para adicionar à carteira
codigos_para_carteira = st.sidebar.multiselect('Selecione os Códigos para Adicionar à Carteira', list(df_filtrado['CÓDIGO'].unique()))
valor_para_investir_carteira = st.sidebar.number_input("Digite o valor para investir na carteira:", min_value=0.0)

if st.sidebar.button("Adicionar à Carteira"):
    adicionar_a_carteira(codigos_para_carteira, valor_para_investir_carteira, carteira)
    st.success(f"{', '.join(codigos_para_carteira)} adicionado(s) à carteira com R$ {valor_para_investir_carteira:.2f} investidos.")
...
# Mostrar rendimentos totais da carteira
if not df_carteira.empty:
    rendimento_total_carteira = df_carteira['Rendimento Total'].sum()
    st.write(f"Rendimento Total da Carteira: R$ {rendimento_total_carteira:.2f}")

# Adicionar gráficos para comparar investimentos
if not df_carteira.empty:
    st.bar_chart(df_carteira.set_index('CÓDIGO')['Rendimento Total'])
